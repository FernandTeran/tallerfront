...
class ListarComponentes extends Component {
    ...
    componentWillMount(){
        ...
        if (lista !== null) {
            lista.map((item,key) => {
                arreglo=arreglo.concat(new this.Obj(item.id_rec,item.observacion,item.id_ubicacion && item.id_ubicacion,item.id_tipo,item.descripcion,item.sigla_programa,item.validado,item.nombre,
...
//crea un objeto para pasar al hijo
    Obj(id_rec,obs,ubic,tipo,descripcion,sigla_programa,validado,nombre,concepto,codigo,numero,importe,fecha,id_alum){
        this.id_rec=id_rec;
        this.obs=obs;
        this.ubic=ubic;
        this.tipo=tipo;
